...
async function startDailyTracking(onUpdate) {
  ...
  try {
    // リスナー登録・計測開始の前に本日歩数で水位を合わせる。
    // ここが 0 のままだと、初回 stepsUpdated で本日分が一気に delta 計上される。
    try {
      lastPolledToday = await getTodaySteps();
    } catch {
      /* 既存の水位を維持 */
    }
    ...
    const today = await getTodaySteps();
    lastPolledToday = Math.max(lastPolledToday, today);
    onStepCallback?.({ sessionSteps: today, deviceTotal: today, delta: 0 });
...
export async function stopNativeSteps() {
  onStepCallback = null;
  // lastPolledToday はリセットしない（停止→再開で本日分を再計上しないため）
...
export function syncPollBaseline(total) {
  lastPolledToday = Math.max(0, Math.floor(Number(total) || 0));
}
