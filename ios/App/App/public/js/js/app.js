...
import { initMap, renderRoute, destroyMap } from './map.js';
...
function init() {
  ...
  pedometer.setCreditedTodayGetter?.(() => {
    const credited = state.pedometerTodaySteps || 0;
    const nativeSynced = getPedometerLastNativeTotal(state);
    return Math.max(credited, nativeSynced);
  });
...
function updateInstallModeWarning() {
  ...
  if (kind === 'android-pwa' || (kind === 'browser' && userMsg.isAndroidBrowser?.())) {
    text.textContent = userMsg.pwaInstallWarning();
...
function initInstallPrompt() {
  ...
  if (isIOS) {
    ...
    installDesc.textContent = 'Safariの「共有」ボタン → 「ホーム画面に追加」';
...
async function restorePedometer() {
  ...
  if (wasTracking) {
    pedometer.restoreSession(
      state.pedometer.sessionSteps || 0,
      state.pedometerTodaySteps || getPedometerLastNativeTotal(state)
    );
  }
...
function handlePedometerUpdate({ sessionSteps, flush, at, pendingDay }) {
  ...
  if (flush && flush > 0) {
    ...
    if (prevDate !== state.todayDate) {
      ...
      state = setPedometerSessionSteps(state, 0);
    }
...
function updatePedometerUI() {
  ...
  $('#pedometer-today').textContent = formatSteps(state.pedometerTodaySteps || 0);

  $('#pedometer-session').textContent = formatSteps(state.pedometer?.sessionSteps || pedometer.getSessionSteps());
...
function checkGoal() {
  if (!state.route || state.goalShown) return;

  const traveledKm = stepsToKm(state.totalSteps, state.strideCm);
  if (traveledKm >= state.route.totalKm) {
    showGoalModal();
  }
}
...
function showGoalModal({ restore = false } = {}) {
  ...
  if (!restore && !savedEntry) {
    state = addToCollection(state, {
      routeId: route.id,
      label: route.label,
...
function fireConfetti() {
  ...
  (function frame() {
    ...
    confetti({
      ...
      colors: ['#0d9488', '#f59e0b', '#14b8a6', '#ffffff']
...
async function onPresetSelect(preset) {
  ...
  if (state.route && state.totalSteps > 0) {

    if (!confirm('現在の旅の進捗はリセットされます。よろしいですか？')) return;
...
function renderCollection() {
  ...
  state.collection.forEach((item) => {
    ...
    const spotLabel = item.spotLabel || dest.spotLabel || item.endName;
    const img = document.createElement('img');
    img.alt = spotLabel;
    img.loading = 'lazy';
    applySpotImage(img, item.endSpotId || dest.spotId, item.endName, item.image || dest.spotImage);
    ...
    body.innerHTML = `
      ...
      <div class="collection-subroute">${escapeHtml(item.label)}</div>
...
function drawMap() {
  ...
  const currentPos = positionOnRoute(getRoutePath(state.route), traveledKm);
